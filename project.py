...
def fbconnect():
        # verify value of state to protect against
...
def categoryListJSON(category_name):
        ...
        items = session.query(MenuItem).filter_by(category_name=category_name).all()
...
@app.route('/category/<string:category_name>/<string:item_title>/delete', methods=['GET', 'POST'])
def deleteItem(category_name, item_title):
        ...
        if login_session['user_id'] != category.user_id:
                return "<script>function myFunction() {alert('You are not authorized to delete menu items . Please create your own items in order to delete items.');}</script><body onload='myFunction()''>"
...
@app.route('/category/<string:category_name>/<string:item_title>/edit', methods=['GET', 'POST'])
...
def newItem():
         ...
         categories = session.query(Category).order_by(asc(Category.name))
         if request.method == 'POST':
                         newItem = MenuItem(title=request.form['title'], description=request.form['description'],category_name=request.form['category_name'])
                         session.add(newItem)
                         session.commit()
